...
def unzip():
    # upzip the crx file
    folder='../raw_data/current'
    # command to unzip
    # $unzip [origin file path] -d [new path]
    unzip_folder='../raw_data/unzip'
...
if __name__ == "__main__":
    ...
    # unzip()
    unzip_folder='../raw_data/unzip_backup'
    permission_file='./all_permission.xls'
    ...
    for root, subdirs, files in tqdm(os.walk(unzip_folder)):
        for file in files:
    ...
    # 
    with open(permission_file_csv,'w') as f:
        csv_f=csv.writer(f)
        csv_f.writerows(permission_items)
