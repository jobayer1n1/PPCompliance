...
if __name__=='__main__':
    ...
    model.load_state_dict(torch.load('./log_v2/EPO:90-ACC:0.932.pth'))
    model.to(device)
    model.eval()
    ...
    print(test_acc)
    report =classification_report(y_pred=res_list,y_true=target_list, output_dict=True)
    df_report = pd.DataFrame(report).transpose()
    df_report.to_csv('result/our_our_report_lstm.csv',index=True)
    # print(report)